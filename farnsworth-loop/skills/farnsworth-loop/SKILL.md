...
- **Preferred — dynamic workflows.** Invoke the bundled Workflow script `workflows/tournament.mjs` (at the plugin root, i.e. `../../workflows/tournament.mjs` relative to this skill's base dir) via the `Workflow` tool, passing `args` (see `references/orchestration.md` for the exact shape: task, mode, runDir, the per-attempt list, and — if any attempt is GLM — `glmRunner` set to `<plugin-root>/bin/glm-run.sh`). The workflow runs Phases 2–5 deterministically — parallel attempts, the blind Opus review, and (two pass) round two and the final rank — and you can watch it live in `/workflows`. It returns the structured mapping + rankings you report in Phase 6. This is opt-in orchestration: the harness will show a confirm at first dispatch. **Anthropic attempts run native; GLM attempts run through the bundled command-runner agents** (`farnsworth-glm-*`) which execute `bin/glm-run.sh` to reach the z.ai endpoint — the Task tool cannot target a GLM model directly, and the runner-script indirection is what makes the GLM path reliable (do not inline the raw `claude`→z.ai command; the wrapper will refuse or shortcut it).
...
**GLM provenance check.** For every GLM attempt, confirm its workspace `_glm_run.log` contains a `FARNSWORTH-GLM-PROVENANCE endpoint=api.z.ai` line (mechanical proof it actually ran on z.ai, not a wrapper faking it with an Anthropic model). Treat a GLM candidate with no provenance marker or no saved deliverable as a failed attempt: exclude it and note the failure in the report. Weaker GLM models (especially `glm-4.5-air`) sometimes fail to save a deliverable; that is an honest failure, not something to paper over.
