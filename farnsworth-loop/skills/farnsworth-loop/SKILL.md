...
- Dogfood backlog: problems found while running tournaments go in `farnsworth-loop/DOGFOOD.md` (a greppable roster), with full evidence one-file-per-item in `docs/dogfood/D-NNNN.md`. Because `.runs/` is gitignored, every item MUST paste a verbatim verdict/guidance excerpt — never rely on a run path. A `@@FL` "dogfood run" claims the top `open` item by pushing a one-row `[in-progress]` claim commit first (git's push race = the lock), fixes it on a branch, fills in `## Resolution`, flips the row to `done`, opens one PR. Full rules: `docs/dogfood/README.md`.
