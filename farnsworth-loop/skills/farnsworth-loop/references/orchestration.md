...
- **Validate**: a candidate must have saved a deliverable AND (for GLM/local) its log must show the **success** provenance contract — the `PROVENANCE` marker *and* `DONE exit=0` *and* no `TIMEOUT`/`ERROR` line (merely "a `FARNSWORTH-` line exists" is not enough; the runners write those before/around failures too). The agent returns the per-candidate `{deliverable, provenance}` as a **schema** (not scraped prose), and the engine **fails closed** — any candidate missing from the return, or not deliverable+provenance, is invalid and excluded *before* the judge runs (recorded `valid:false` + reason).
