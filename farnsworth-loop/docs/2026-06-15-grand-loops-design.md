# Farnsworth Loop — Grand Loops (`Z`) + Prose-Inferred `N`: evaluation & build plan
...
- **Explicit-N-vs-prose conflict.** `@@FL:4 ... 2 opus, 2 glm, 1 codex` says N=4 by sigil and N=5 by prose. This is the skill's cardinal sin (silent guessing) if mishandled. **Resolution: stop and ask, surfacing both numbers; proceed silently only when they are equal.** Never silently override an explicit integer (neither direction — not "prose wins", not "N caps prose").
...
- **Non-implementable tasks.** `@@FL:4:1:3` on "write a haiku" would run a tournament, then try to "implement a haiku into the project" and open an empty/nonsense PR three times. Must **detect non-implementable tasks before entering grand-loop mode.**
...
- **Branch-naming convention conflicts with a global user rule.** The user's global CLAUDE.md says branches are prefixed `rob/`. The required convention here is `FL-<n>-<random7>`. **This is a real conflict and must be called out, not silently resolved** — the feature-specific convention overrides the global rule *for these loop branches only*, and the skill must say so.
...
```
@@FL[:N][:M[:Z]]      (case-insensitive; optional spaces around every colon)
```
...
Backwards compatibility is total: `@@FL:5` and `@@FL:5:2` parse to `Z=1` and behave identically to today. `Z` can appear only when `M` is present (positional: you cannot write `@@FL:5::3`-style skips — if you want default M with explicit Z, write `@@FL:5:1:3`).

**`N` optional:** `@@FL::2:3` (empty N, M=2, Z=3) or `@@FL:` followed by a prose spec means "infer N from prose." If N is empty in the sigil, a prose model spec **must** be present, else ask for N. The empty-segment form is awkward; the common path is `@@FL` with a prose spec and no N at all (see prose grammar).
...
```
SIGIL   = /@@FL\s*(?::\s*(\d+)?)?(?:\s*:\s*(\d+))?(?:\s*:\s*(\d+))?/i
                     └ N(opt)        └ M(opt)        └ Z(opt)
PROSE   = /farnsworth\s+loop\s*:\s*(\d+)(?:\s*:\s*(\d+))?(?:\s*:\s*(\d+))?/i
                                   └ N      └ M(opt)        └ Z(opt)
...
|---|---|---|---|---|---|
| `do abc @@FL:5` | 5 | 1 single | 1 | from Phase-1 gate | yes (today) |
| `do abc @@FL:5:2` | 5 | 2 two | 1 | from gate | yes |
| `do abc @@FL:5:2:3` | 5 | 2 two | 3 | from gate | yes + **autonomy auth** |
| `improve X @@FL:4:1:3 run with 2 opus, 2 glm, 1 codex` | **conflict** 4 vs 5 → STOP & ask | 1 | 3 | (pending answer) | ask |
| `improve X @@FL:1:3 2 opus 2 glm 1 codex` (N omitted) | 5 (from spec) | 1 single | 3 | `[opus,opus,glm-5.2,glm-5.2,codex-medium]` | no Phase-1 menu (spec answers it) + autonomy auth |
| `do abc farnsworth loop:4` | 4 | 1 single | 1 | from gate | yes |
| `do abc, run with 2 opus and 1 sonnet farnsworth loop:3:2` | 3 (agree) | 2 two | 1 | `[opus,opus,sonnet]` | no menu |
...
- **The tournament produces a PROPOSAL; a separate implementer agent applies the winner:** keeps the engine pure and unchanged (attempts stay sandboxed one-shots producing a written proposal, exactly as today), and confines **all** real-repo writes to a single auditable implementer actor on one FL- branch. The implementer reads the winning proposal + the real repo and makes the edits.

**Decision: proposal + separate implementer.** The tournament's job (when `Z>1`) is to produce the best *implementation proposal* (a concrete, file-level change description, ideally with diffs in fenced blocks, in its proposal artifact). The implementer turns the winning proposal into a real diff on the FL- branch. Attempt fan-out stays sandboxed; exactly one actor writes to the repo.
...
for k in 1..Z:
    ...
    git switch base
    git switch -c "FL-<k>-<random7>"        # §7 suffix; FAN: always off base
    ...
    Task(implementer, { proposal: winnerProposalPath, repoRoot: base_repo, branch: "FL-<k>-..." })

    # 3) verify (fail-closed, §4 fork c)
    ok = run_verify(detected_commands)
    git add -A && git commit -m "FL loop <k>: <one-line winner summary>"

    # 4) push + PR
    git push -u origin "FL-<k>-<random7>"
    if ok:
        gh pr create --base base --head "FL-<k>-..." --title "FL loop <k>: <summary>" --body <template §7.3>
    else:
        gh pr create --draft --label needs-human --base base --head "FL-<k>-..." --title "[needs-human] FL loop <k>" --body <template + failing output>
...
STACK variant: `base` for loop k becomes branch of loop k-1 (`git switch -c FL-<k>-... FL-<k-1>-...`), and `--continue-on-failure` is disallowed.
...
A new bundled agent (`agents/farnsworth-implementer.md`, model: a strong Anthropic model — Sonnet or Opus, since it writes real code) invoked via Task with `Bash Read Write Edit` in the **real repo root** on the FL- branch. Its brief:
...
- If the proposal is ambiguous or under-specified, implement the most faithful reasonable interpretation
  and note the assumption in a file `FL-NOTES.md` at repo root.
- Leave the working tree with your changes unstaged; end with a 3-6 line summary of what you changed.
```

It is the *only* actor that writes to the real repo, and it does not touch git — keeping the auditable git/PR mechanics entirely in the driver. This is the design-around for INV-2: real writes are real, but confined to one actor on one FL- branch, never to `main`, never auto-merged.
...
This is UNATTENDED and WRITES TO A REAL REPOSITORY. For each of 3 loops I will:
  • create a new branch  FL-<n>-<random7>  off  <base = current branch 'main'>   (FAN topology)
...
Branch naming:  FL-<loop>-<random7>  (NOTE: this overrides your global 'rob/' branch-prefix rule
...
- **Friction proportional to blast radius:** the user must **re-type Z** to confirm (a fat-fingered `@@FL:5:2:30` doesn't run 30 repo-mutating loops on one Enter).
...
fl_suffix() { LC_ALL=C tr -dc '0-9a-z' < /dev/urandom | head -c 7; echo; }
# branch name for loop k:
fl_branch() { echo "FL-$1-$(fl_suffix)"; }   # e.g. FL-1-fhjdks6, FL-2-fhdsjy4
```
...
```bash
git switch "<base>"
git switch -c "$(fl_branch "$k")"
# (implementer applies changes here)
git add -A
git commit -m "FL loop ${k}: ${WINNER_SUMMARY}"
...
gh pr create --base "<base>" --head "$BRANCH" \
  --title "FL loop ${k}: ${WINNER_SUMMARY}" \
  --body "$(cat <<EOF
## Farnsworth Loop — grand loop ${k} of ${Z}

**Automated, unverified-by-human change.** This branch was produced by an unattended
Farnsworth Loop grand-loop run. A human must review before merging. NOT auto-merged.
...
| File | Change |
|---|---|
| `skills/farnsworth-loop/SKILL.md` | Phase 0: add the `Z` segment + prose-spec parsing + the conflict-stop rule (§2.5) + non-implementable detection (§5.4). New **Phase 0b: grand-loop authorization** (§6) replacing the per-dispatch gate when `Z≥2`. New **Phase 7: grand-loop driver procedure** (§5.2 control flow, FAN default, fail-closed verify, PR mechanics). Note the `FL-` vs global `rob/` conflict. Update Quick reference table + description frontmatter (mention `@@FL:N:M:Z` and prose model spec). Keep all `Z=1` behavior byte-identical. |
| `skills/farnsworth-loop/references/orchestration.md` | New section "Grand loops" documenting the driver/engine/implementer layering (§3), the implementer agent brief (§5.5), cross-loop memory (§5.3), verify auto-detection, idempotency (§7.4). State explicitly the engine is unchanged. |
...
| `bin/grand-loop.sh` (**new**) | Deterministic helpers the SKILL procedure calls: `fl_suffix`, `fl_branch`, `preflight`, `detect_verify`, `run_verify`, `commit_and_push`, `open_pr` (normal + draft/needs-human), `stop_file_check`, `done_marker`. All git/gh lives here, never improvised by an agent. |
| `bin/fl-parse` (**new**, standalone) | Testable parser+normaliser (§2): input = raw message; output = JSON `{task, n, mode, z, assignment[], conflict?, errors[]}`. Isolated so the grammar/normaliser are **unit-testable** without running a tournament. The SKILL calls it in Phase 0; it is the single source of truth for the grammar. |
| `agents/farnsworth-implementer.md` (**new**) | The implementer agent (§5.5), model Sonnet/Opus, `Bash Read Write Edit`, applies winner proposal, no git. |
| `plugin.json` | Add `farnsworth-implementer` to `components.agents`; bump version (2.0.0 — new major capability). Update description to mention grand loops + prose model spec. |
...
```json
{"query": "improve the error handling @@FL:4:2:3", "should_trigger": true, "mode": "two", "n": 4, "z": 3},
{"query": "refactor the parser @@FL:5:1:2", "should_trigger": true, "mode": "single", "n": 5, "z": 2},
{"query": "optimise this loop, farnsworth loop:4:2:3", "should_trigger": true, "mode": "two", "n": 4, "z": 3},
{"query": "improve X @@FL:1:3 run with 2 opus, 2 glm 5.2, 1 codex high", "should_trigger": true, "mode": "single", "z": 3, "n": 5, "assignment": ["opus","opus","glm-5.2","glm-5.2","codex-high"]},
{"query": "do abc, 1 opus and 1 sonnet and 1 haiku farnsworth loop:3", "should_trigger": true, "mode": "single", "n": 3, "z": 1, "assignment": ["opus","sonnet","haiku"]},
{"query": "improve X @@FL:4:1:3 run with 2 opus, 2 glm, 1 codex", "should_trigger": true, "conflict": true, "note": "N=4 (sigil) vs N=5 (spec) -> STOP and ask"},
{"query": "tidy things up @@FL:3:1:9", "should_trigger": true, "z": 9, "note": "Z>Z_MAX(5) -> refuse, ask to split"},
{"query": "write a haiku @@FL:4:1:3", "should_trigger": true, "z": 3, "note": "non-implementable -> offer Z=1 instead of opening empty PRs"},
...
{"query": "create a branch FL-1-abc1234 and open a PR", "should_trigger": false, "note": "branch ops, not a tournament"},
...
- OK that `FL-<n>-<random7>` overrides your global `rob/` prefix for loop branches?
...
- **Mid-loop interruption is detect-and-stop, not resume.** The engine has no resumability; a chain that dies leaves a `DONE`-less FL- branch for the human to inspect. We do not auto-resume a half-applied implementer step (unsafe).
...
- **Two harness facts are honour-system, not enforced** (carried from today): blind judges *could* read sibling dirs; and the engine's purity protects only the tournament, not the new implementer — which is exactly why the implementer is a single audited actor on an isolated FL- branch and never merges.
```
