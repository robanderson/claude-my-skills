...
When `bin/fl-issue.sh new` runs with `gh` unavailable (no network, unauthenticated, headless/cron,
...
- When `gh` is reachable again, run `bin/fl-issue.sh drain-inbox`; it lists the drafts to re-file via
  `bin/fl-issue.sh new …` (which re-validates evidence and dedups), then `git rm` each filed draft.
