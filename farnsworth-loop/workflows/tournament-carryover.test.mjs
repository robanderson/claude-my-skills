...
const GOOD_GLM_LOG =
  'FARNSWORTH-GLM-PROVENANCE endpoint=api.z.ai flag=--model opus\n' +
  '...work...\n' +
  'FARNSWORTH-GLM-DONE exit=0\n'
...
function runGate({ dispatch, carriedOver, writeDeliverable, writeLog, logContent }) {
  const root = mkdtempSync(join(tmpdir(), 'fl-d0004-'))
  try {
...
echo "FLV X d=$([ "$D" -gt 0 ] && echo 1 || echo 0) p=$P"
`
    const out = execSync(script, { shell: '/bin/sh', encoding: 'utf8' }).trim()
    const m = out.match(/FLV X d=(\d) p=(\d)/)
...
{
  const badLog = 'FARNSWORTH-GLM-PROVENANCE endpoint=api.z.ai\nFARNSWORTH-GLM-TIMEOUT secs=300\nFARNSWORTH-GLM-DONE exit=124\n'
...
check('(pure) native snippet is exactly "P=1"', provCheckShell('', '', '', false) === 'P=1')
{
  ...
  check('(pure) non-carryover runner snippet still greps the anchored success contract',
    normal.includes("grep -q '^FARNSWORTH-CODEX-PROVENANCE endpoint='") &&
    normal.includes("grep -q '^FARNSWORTH-CODEX-DONE exit=0'") &&
    normal.includes("! grep -q '^FARNSWORTH-CODEX-\\(TIMEOUT\\|ERROR\\)'"))
}
