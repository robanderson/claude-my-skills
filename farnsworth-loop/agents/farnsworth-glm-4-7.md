...
4. Run the brief on glm-4.7, agentic, with cwd = the workspace, by executing:
   ...
   ( cd "<workspace>" && \
     ...
     claude -p "$(cat _brief.txt)" --model sonnet --allowedTools "Bash Read Write Edit" \
   ) > "<workspace>/_glm_run.log" 2>&1
   ```

   (`--model sonnet` is what selects glm-4.7 on the z.ai endpoint — do not change it.)
