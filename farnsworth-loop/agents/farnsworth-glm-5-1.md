...
4. Run the brief on glm-5.1, agentic, with cwd = the workspace, by executing:
   ...
   ( cd "<workspace>" && \
     ...
     claude -p "$(cat _brief.txt)" --model glm-5.1 --allowedTools "Bash Read Write Edit" \
   ) > "<workspace>/_glm_run.log" 2>&1
   ```

   (`--model glm-5.1` is what selects glm-5.1 on the z.ai endpoint — do not change it.)
