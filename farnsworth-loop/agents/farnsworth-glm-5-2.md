...
4. Run the brief on glm-5.2, agentic, with cwd = the workspace, by executing:
   ...
   ( cd "<workspace>" && \
     ...
     claude -p "$(cat _brief.txt)" --model opus --allowedTools "Bash Read Write Edit" \
   ) > "<workspace>/_glm_run.log" 2>&1
   ```

   (`--model opus` is what selects glm-5.2 on the z.ai endpoint — do not change it.)
