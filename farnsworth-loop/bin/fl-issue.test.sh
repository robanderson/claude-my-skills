#!/usr/bin/env bash
# =============================================================================
# fl-issue.test.sh — no-network regression tests for fl-issue.sh's evidence
...
#
# Run:  bash bin/fl-issue.test.sh   (exit 0 = all pass)
# =============================================================================
set -uo pipefail
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SUT="$HERE/fl-issue.sh"
...
echo "fl-issue.sh check-evidence guards:"
...
expect_rc 0 "clean prose"      'fl-parse mis-read the prose "two pass" form and ran single pass.'

echo
echo "fl-issue guards: $pass passed, $fail failed"
[ "$fail" -eq 0 ]
